...
bool BatchGenerator::append(const Problem &problem, int verbose)
{
    ...
    for(int i = 0; i < problem.num_vars; ++i)
        this->splits_vars.push_back(i);
    for(int i = 0; i < (int)problem.sat_problem.size(); ++i)
        this->splits_clauses.push_back(i);
    ...
    sat_assert_debug(num_vars == (int)(splits_vars.size()));
    sat_assert_debug(num_clauses == (int)(splits_clauses.size()));
...
PyObject *BatchGenerator::to_python() const
{
    ...
    PyTuple_SetItem(result, 4, this->py_splits(this->splits_vars));
    PyTuple_SetItem(result, 5, this->py_splits(this->splits_clauses));
    return result;
}

PyObject *BatchGenerator::py_splits(const std::vector<int> &splits) const
{
    npy_intp sizes_i[] = {(npy_intp)splits.size()};
    PyObject *result = Wrap_PyArray_SimpleNew(1, sizes_i, NPY_INT32);
    int *data_i = (int *)PyArray_DATA((PyArrayObject *)result);
